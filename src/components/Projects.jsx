...
const ProjectCard = ({ title, stack, features, githubLink, liveLink, image, aosDelay, index }) => {
  ...
  const handleMouseMove = useCallback((e) => {
    ...
    setGlareStyle({
      opacity: 0.2,
      background: `radial-gradient(circle at ${x * 100}% ${y * 100}%, rgba(0,255,204,0.3), transparent 50%)`,
    });
    setBorderGlow({
      boxShadow: `${(x - 0.5) * 30}px ${(y - 0.5) * 30}px 60px rgba(0, 255, 204, 0.15), 0 0 20px rgba(0, 255, 204, 0.1)`
    });
  }, []);
  ...
  return (
    ...
    >
      <div 
        className="glow-card bg-[#12131a]/80 border border-white/[0.08] rounded-3xl p-6 md:p-8 backdrop-blur-2xl shadow-[0_10px_50px_rgba(0,0,0,0.5)] hover:border-[#00ffcc]/30 transition-all duration-500 relative group overflow-hidden flex flex-col h-full"
        style={borderGlow}
      >
        <div 
        ...
        <div className="absolute top-0 right-0 w-32 h-32 bg-gradient-to-bl from-[#00ffcc]/10 to-transparent pointer-events-none rounded-tr-3xl z-0"></div>

        <div className="absolute top-6 right-6 text-[80px] font-black text-white/[0.03] leading-none pointer-events-none z-0 select-none" style={{ fontFamily: "'Courier New', Courier, monospace" }}>
          0{index + 1}
        </div>
        ...
        {image && (
          <div className="relative w-full h-48 md:h-56 mb-6 rounded-2xl overflow-hidden z-10 border border-white/5" style={{ transform: 'translateZ(30px)' }}>
            ...
            <div className="absolute bottom-0 left-0 right-0 h-1 bg-gradient-to-r from-[#00ffcc] to-[#00b8ff] opacity-0 group-hover:opacity-100 transition-opacity duration-500"></div>
          </div>
        )}

        <div className="relative z-10 flex justify-between items-start mb-5" style={{ transform: 'translateZ(20px)' }}>
          <h3 className="text-xl md:text-2xl font-outfit font-black text-white group-hover:text-transparent group-hover:bg-clip-text group-hover:bg-gradient-to-r group-hover:from-[#00ffcc] group-hover:to-[#00b8ff] transition-all duration-500 tracking-tight leading-tight pr-4">{title}</h3>
          <div className="flex gap-2">
            {githubLink && (
              <a href={githubLink} target="_blank" rel="noopener noreferrer" className="text-gray-500 hover:text-white transition-all duration-300 p-2.5 bg-white/5 rounded-xl hover:bg-[#00ffcc] hover:shadow-[0_0_20px_rgba(0,255,204,0.4)] shrink-0" title="Source Code">
            ...
            {liveLink && (
              <a href={liveLink} target="_blank" rel="noopener noreferrer" className="text-gray-500 hover:text-white transition-all duration-300 p-2.5 bg-white/5 rounded-xl hover:bg-[#00ffcc] hover:shadow-[0_0_20px_rgba(0,255,204,0.4)] shrink-0" title="Live Demo">
        ...
        <div className="relative z-10 flex flex-wrap gap-2 mb-6" style={{ transform: 'translateZ(15px)' }}>
          {stack.map((tech, idx) => (
            <span key={idx} className="px-3 py-1 bg-white/[0.04] text-gray-300 border border-white/[0.08] text-[11px] font-bold rounded-lg hover:bg-[#00ffcc]/15 hover:text-[#00b8ff] hover:border-[#00ffcc]/30 transition-all duration-300 cursor-default">
              {tech}
            </span>
          ))}
        </div>
        ...
        <ul className="relative z-10 space-y-3 text-gray-400 font-medium list-none flex-1" style={{ transform: 'translateZ(10px)' }}>
          {features.map((feature, idx) => (
            <li key={idx} className="leading-relaxed text-[13px] md:text-sm flex items-start gap-3 group/item">
              <svg className="w-4 h-4 text-[#00ffcc] shrink-0 mt-0.5 opacity-60 group-hover/item:opacity-100 transition-opacity" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2.5" d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z"></path></svg>
...
const Projects = () => {
  return (
    <section id="projects" className="bg-[#0B0C10] py-32 px-6 md:px-12 w-full relative overflow-hidden font-sans border-b border-white/5">
      <div className="absolute top-[10%] left-[-10%] w-[500px] h-[500px] bg-[#00ffcc]/8 rounded-full blur-[150px] pointer-events-none"></div>
      <div className="absolute bottom-[10%] right-[-5%] w-[400px] h-[400px] bg-[#b100ff]/5 rounded-full blur-[120px] pointer-events-none"></div>
      ...
      <div className="max-w-6xl mx-auto relative z-10">
        <div data-aos="fade-up" className="mb-20 text-center">
          <div className="inline-flex items-center gap-2 border border-white/10 rounded-full px-5 py-1.5 text-sm text-gray-300 font-bold mb-6 shadow-sm bg-white/5 backdrop-blur-md">
            <svg className="w-4 h-4 text-[#00ffcc]" fill="currentColor" viewBox="0 0 24 24"><path d="M12 2L2 7l10 5 10-5-10-5zM2 17l10 5 10-5M2 12l10 5 10-5"/></svg>
            Portfolio
          </div>
